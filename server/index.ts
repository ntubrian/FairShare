  ...
  <body>
    ...
    <script>
      (function initEmbeddedSandbox() {
        ...
        var initialEndpoint = window.location.origin + ${JSON.stringify(
          graphqlPath
        )};
  ...
  createHandler({
    ...
    context: async (request): Promise<GraphQLContext> => {
      ...
      return {
        viewer,
        acceptLanguage:
          typeof acceptLanguage === "string" ? acceptLanguage : "en",
      };
    },
  })
);
...
app.listen(port, () => {
  ...
  if (env.graphqlSandboxEnabled) {
    console.log(
      `Apollo Sandbox redirect: http://localhost:${port}${sandboxPath}`
    );
  }
});
